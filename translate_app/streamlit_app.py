...
if st.button("Submit"):
    with st.spinner("Translating ..."):
        # eng-spanish translation
        encoder, decoder = load_models()
        if beam_width!=None:
            spanish_sequences = decode_sequence_beam_search(encoder, decoder, input_text, beam_width=beam_width)
            ...
            st.subheader("Predicted Spanish Translation")
            spanish_seq = decode_sequence(encoder, decoder, input_text)[:-4]
...
st.markdown("The interactive app is created using [Streamlit](https://streamlit.io/), an open-source framework that lets users creating apps for machine learning projects very easily."
)
